...
} from "src/app/stepper/stepper-dialog/stepper.dialog.component";
...
export class StepperComponentService {
  ...
  public downloadLabels(labels: DownloadLabelsDTO) {
    return this.httpClient
      .put(APIs.downloadLabels, labels, { responseType: "blob" })
      .pipe(
        catchError(() => {
  ...
  public downloadHistoryDoc() {
    return this.httpClient
      .get(APIs.downloadHistoryDoc, { responseType: "blob" })
      .pipe(
        catchError(() => {
  ...
  public importAndGetStepperData(
    uploadImageData: FormData
  ): Observable<ImportDTO> {
    return this.httpClient.post<ImportDTO>(APIs.import, uploadImageData).pipe(
      catchError((err) => {
  ...
  private handleImportErrors(err: {
    status: number;
    error: ImportDTO;
  }): Observable<never> {
    if (err.status === 400) {
  ...
  private getSettingsFromUser(products: string[]) {
    if (products.length === 0) {
      this.snackBar.showAndRemain(
        "Παρακαλώ τραβήξτε το τιμολογιο ξανα.",
        "κλείσιμο"
      );
    } else {
      var p = products.pop();
  ...
  public updateRetailPrices(data: Product[], date: string): Observable<any> {
    ...
    return this.httpClient.put(APIs.updatePrices, dto).pipe(
      catchError(() => {
        this.snackBar.showError(
          "Αποτυχία ενημέρωσης τιμών. Προσπαθήστε ξανά",
          "Ok"
        );
        return throwError("Failed to update the prices.");
      })
    );
  }
}
